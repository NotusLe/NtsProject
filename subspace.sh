...
User=$USER
Type=simple
ExecStart=$(which subspace-node) --chain gemini-1 --execution wasm --in-peers 10 --out-peers 10 --max-parallel-downloads 10 --keep-blocks 1024 --pruning archive --validator --name $SUBSPACE_NODENAME
Restart=on-failure
LimitNOFILE=65535
